 ...
 */
export function resetMobileBridgeHandler(req: Request, res: Response): void {
  const schemeRaw = (process.env.MOBILE_APP_SCHEME ?? DEFAULT_SCHEME).trim();
  const scheme = schemeRaw.replace(/:$/, "") || DEFAULT_SCHEME;
